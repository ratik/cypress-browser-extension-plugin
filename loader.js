...
function onBeforeBrowserLaunch(browser = {}, opts) {
  return whenAllBuilt().then(() => {
    ...
    if (toLoad.length > 0) {
      ...
      const existingLoadArgIndex = opts.args.findIndex(arg => (
        (typeof arg === 'string') && arg.startsWith('--load-extension=')
      ));
      if (existingLoadArgIndex >= 0) {
        // eslint-disable-next-line no-param-reassign
        opts.args[existingLoadArgIndex] = `${opts.args[existingLoadArgIndex]},${dirList}`;
      } else {
        opts.args.push(`--load-extension=${dirList}`);
      }
    }
    return opts;
  });
